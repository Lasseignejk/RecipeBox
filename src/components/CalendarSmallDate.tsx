...
const CalendarSmallDate = ({date}:CalendarProps):JSX.Element => {
    ...
    const todayClasses = date.today == true ? "border-2 border-lightPrimary text-lightPrimary font-bold rounded-md" : "border-2 border-lightSurf text-lightSecondary"
  return (
		<div
			data-testid="date"
			className={`flex flex-col w-full items-center  ${todayClasses} `}>
			<p className={`text-sm`}>{dayOfWeek}</p>
			<p className={`text-xl`}>{dateNum}</p>
		</div>
  );
}
